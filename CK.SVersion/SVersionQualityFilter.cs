...
public readonly struct SVersionQualityFilter
{
    ...
    public SVersionQualityFilter( string? min, bool includeMin, string? max, bool includeMax, bool allowCI )
    {
        ...
        if( max != null ) ArgumentOutOfRangeException.ThrowIfNegative( SVersion.ComparePrerelease( max, min ) );
        _min = min;
    ...
    /// (angle brackets are parentheses when <see cref="IncludeMin"/> or <see cref="IncludeMax"/> are false).
    /// </summary>
