...
public static class SVersionLockExtension
{
    ...
    /// <summary>
    /// Tries to parse one of the <see cref="SVersionLock"/> terms.
    /// Note that parsing is case insensitive and that all "Lock" wan be written as "Locked".
    /// </summary>
    /// <param name="s">The string to parse.</param>
    /// <param name="l">The read lock.</param>
    /// <returns>True on success, false otherwise.</returns>
    public static bool TryParse(ReadOnlySpan<char> s, out SVersionLock l)
    {
        l = SVersionLock.NoLock;
        return TryMatch(ref s, ref l) && s.Length == 0;
    }
    ...
    /// <param name="l">The version lock on sucess. On failure, the value is unchanged.</param>
    /// <returns>True on success, false otherwise.</returns>
