...
public readonly partial struct SVersionBound
{
    static (SVersion? Version, int FMajor, int FMinor, string? Error) TryMatchFloatingVersion( ref ReadOnlySpan<char> s )
    {
        ...
        if (SVersion.TryMatch(ref s, out var version, checkBuildMetaDataSyntax: false, allowPrefix:false ) )
        {
        ...
        return (null, 0, 0, version?.ErrorMessage ?? "Pattern not matched.");
    }

    static (ParseResult Result, bool IsFloatingMinor) TryMatchRangeAlone( ref ReadOnlySpan<char> s, SVersionLock defaultLock, bool includePrerelease )
    {
        var r = TryMatchFloatingVersion( ref s );
        if( r.Error != null ) return (new ParseResult( r.Error ), false);

        bool allowCI = includePrerelease;
        string minPrerelease = includePrerelease ? "0" : "";
        if( r.Version != null )
        {
            ...
            //
            // We also AllowCI regardless of the includePrerelease.
            // If we are coming from the ~ (tilde range), the lock is on the patch, not on the minor, and this exactly matches the
            // npm behavior.
            //
            ...
            if( r.Version.IsPrerelease )
            {
               allowCI = true;
                ...
                {
                    defaultLock = SVersionLock.LockPatch;
                    isApproximated = false;
                }
            }
            return (new ParseResult( new SVersionBound( r.Version, defaultLock, minPrerelease, allowCI ), isApproximated ), false);
        }
        if( r.FMajor < 0 ) return (NpmVersionBoundAll( includePrerelease ), false);
        if( r.FMinor < 0 ) return (new ParseResult( new SVersionBound( SVersion.Create( r.FMajor, 0, 0 ), SVersionLock.LockMajor, minPrerelease, allowCI ), isApproximated: !includePrerelease ), true);
        return (new ParseResult( new SVersionBound( SVersion.Create( r.FMajor, r.FMinor, 0 ), SVersionLock.LockMinor, minPrerelease, allowCI), isApproximated: !includePrerelease ), false);
    }
    ...
    static ParseResult NpmVersionBoundAll( bool includePrerelease )
    {
        return includePrerelease
                ...
                : new ParseResult( new SVersionBound( _000Version, SVersionLock.NoLock, "", allowCI: false ), isApproximated: true );
    }
